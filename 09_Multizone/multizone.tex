...
\begin{equation}
\begin{aligned}
\pdv{n}{t}=&-\nabla\cdot\qty(n\boldsymbol{v}_A)-\nabla\qty(\overline{\overline{D}}\cdot\nabla n)+\pdv{ }{\gamma}\qty(\dot{\gamma}n)-\pdv{ }{\gamma}\qty(\frac{\gamma}{3}\qty(\nabla\cdot\boldsymbol{v}_A)n) \\
&\pdv{ }{\gamma}\qty(\gamma^2 D_{\gamma\gamma}\pdv{ }{\gamma}\qty(\frac{n}{\gamma^2})) + S\qty(\gamma,\boldsymbol{r},t)\text{ ,}      
\end{aligned} \label{eq:chapter_9_full_numerical_diffusion_equation}
\end{equation} 
\noindent where the first term in \autoref{eq:chapter_9_full_numerical_diffusion_equation} describes the evolution of cosmic rays due to a co-moving fluid with velocity $\boldsymbol{v}_A$. The second term considers the diffusion of cosmic rays where $\overline{\overline{D}}$ is a second rate tensor describing the rate of diffusion in 3D. The third and fourth term represents losses due to radiative processes (see chapter \autoref{sec:chapter1_non_thermal_emission}) and adiabatic expansion respectively. The fifth term describes diffusion as a result of the re-acceleration of cosmic rays due to stochastic process. Finally, $S\qty(\gamma,\boldsymbol{r},t)$, is the source term of high-energy cosmic rays.
\par~\par
...
\begin{equation}
\begin{aligned}
\pdv{n}{t}&=-\nabla\qty(D\qty(\boldsymbol{r},\gamma)\cdot \nabla n) + \pdv{ }{\gamma}\qty(\dot{\gamma}n) + S\qty(\gamma,\boldsymbol{r},t) \\
&= \qty(\pdv{n}{t})_\text{diffusion} + \qty(\pdv{n}{t})_\text{radiative} + \qty(\pdv{n}{t})_\text{source term}\text{ ,} 
\end{aligned} \label{eq:chapter_9_iso_diffusion_numerical_diffusion_equation}
\end{equation} 
\noindent where the time evolution of the cosmic ray distribution is described by the sum of the evolution due to diffusion, radiative cooling and a cosmic ray source term. Note that \autoref{eq:chapter_9_iso_diffusion_numerical_diffusion_equation} is similar to \autoref{eq:chapter_7_cr_energy_distribution}, where the `escape term' has been replaced with an expression describing the losses due to diffusion. For a simple case of isotropic diffusion ($\overline{\overline{D}}=D$ and $\nabla D=0$), the diffusion term in \autoref{eq:chapter_9_iso_diffusion_numerical_diffusion_equation} becomes: 

\begin{equation}
    \begin{aligned}
    \qty(\pdv{n}{t})_\text{diffusion}&=-\nabla\qty(D\qty(\boldsymbol{r},\gamma)\cdot \nabla n)  \\
    &=-\nabla D\cdot \nabla n -D\nabla^2 n \\
    &=-D\nabla^2 n\text{ ,} 
    \end{aligned}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        \begin{split}
            \qty(\pdv{n}{t})_\text{diffusion}\bigg|
            \begin{array}l
                 \gamma \\
                 t \\
                 x,y,z
            \end{array}
            = \sum_\qty{i=x,y,z} &\left[ \boldsymbol{D}^*\bigg|
            \begin{array}{l}
                \gamma \\
                t-\Delta t \\
                i+\Delta i/2
...
\begin{equation}
    \begin{aligned}
    \boldsymbol{\boldsymbol{D}}^*\bigg|\begin{array}{l}
                \gamma \\
                t-\Delta t \\
                i\pm\Delta i/2
            \end{array}
            &=\chi D_0 \qty(\frac{\gamma m_e}{1~\TeV})^\delta \qty(\frac{B_i + B_{i\pm 1}}{2\times 3~\si{\micro G}})^{-\delta} \frac{\Delta t}{\Delta i^2}\text{ ,} 
...
\item To ensure that the electron distribution stays positive ($n\geq 0$) and the position of the maximum value of $\boldsymbol{D}=D\Delta t/\Delta x^2$ remains constant across time:
...
\begin{subequations}
    \begin{alignat}{1}
    n_\text{max}\qty(1-6\boldsymbol{D})&\geq n_\text{max} \boldsymbol{D} \label{eq:chapter_9_time_condition1} \\
    \boldsymbol{D}_\text{max}&\leq \frac{1}{7}\text{ ,} 
    \end{alignat}
\end{subequations}

\begin{enumerate}[label=\textbf{\arabic*}]
\item[] where the factor six in \autoref{eq:chapter_9_time_condition1} comes from the three deggrees of freedom. Hence the time step becomes:
\end{enumerate}

\begin{equation}
    \begin{aligned}
    \Delta t = \frac{\Delta x^2}{\boldsymbol{D}_\text{max}}\text{ ,} 
    \end{aligned} \label{eq:09_Multizone_time_step}
\end{equation}

\begin{enumerate}[label=\textbf{\arabic*}.]\setcounter{enumi}{\value{counter1}}
\item[] where $\boldsymbol{D}_\text{max}$ is the maximum value of $\boldsymbol{D}$ across the 3D grid. Note that \autoref{eq:09_Multizone_time_step} satisfies the condition where the finite difference method is stable (\autoref{eq:09_finite_difference_diff_stable}).
...
\begin{equation}
    \begin{aligned}
    n\qty(\gamma,t) &=\frac{\dot{\gamma}_0}{\dot{\gamma}}n\qty(\gamma_0,t-\Delta t)\text{ ,} 
    \end{aligned}
\end{equation}
...
\begin{enumerate}[label*=\textbf{\alph*}.]\setcounter{enumii}{\value{counter2}}
    \itemsep0em
    \item[] where $\gamma_0$ is the Lorentz factor of the electron at $t=t-\Delta t$ before cooling to Lorentz factor $\gamma$ (see \autoref{eq:chapter_7_cooling_time}). 
    \item For each voxel, the electron energy distribution at time $t$ is found by incrementing the electron energy distribution from step \ref{item:09_step_5a} with the net transfer of electrons from neighbouring voxels with (see \autoref{eq:chapter_8_multizone_diff_numerical_sol}).
...
\begin{equation}
	\begin{aligned}
		n\qty(x,y,z,\gamma,t)&=\iiint_{x,y,z=-\infty}^{\infty} n_\text{green}\qty(\boldsymbol{r}-\boldsymbol{s_r})R_\text{source}\qty(\boldsymbol{s_r})\dd{\boldsymbol{s_r}} \text{ ,} 
...
\begin{equation}
    \begin{aligned}
    R_\text{source}\qty(\boldsymbol{s_r})=
    \begin{cases}
...
\begin{equation}
    \begin{aligned}
        n_\text{green}\qty(\gamma,r,t)&=\frac{\qty(1-\gamma b_st)^{\Gamma-2}A\gamma^{-\Gamma}}{\pi^{\frac{3}{2}}R_\text{diff}\qty(\gamma,t,B)^3}\exp\qty(-\frac{r^2}{R_\text{diff}\qty(\gamma,t,B)^2}) \\
...
\begin{equation}
	\begin{aligned}
		n_\text{exact}\qty(x,y,z,\gamma,t)&=\int_{x=-2}^{2}\int_{y=-2}^{2}\int_{z=-2}^{2}\frac{n_0}{64\pi^{\frac{3}{2}}R_\text{diff}^3}\prod_{i=x,y,z} \exp(-\frac{\qty(i-s_i)^2}{R_\text{diff}^2})\dd{s_i}\text{ ,} 
...
\begin{equation}
	\begin{aligned}
		n_\text{exact}\qty(x,y,z,\gamma,t)&=\frac{n_0}{64\pi^{\frac{3}{2}}}\prod_{i=x,y,z}\int_{\frac{i-2}{R_\text{diff}}}^{\frac{i+2}{R_\text{diff}}}\exp(-u_i^2)\dd{u_i}\text{ .} 
...
\begin{equation}
	\begin{aligned}
		n_\text{exact}\qty(x,y,z,\gamma,t)&=\frac{n_0}{64\pi^{\frac{3}{2}}}\prod_{i=x,y,z}\int_{\frac{i-2}{R_\text{diff}}}^0\exp(-u_i^2) + \int_{0}^{\frac{i+2}{R_\text{diff}}}\exp(-u_i^2) \\
...
\begin{wrapfigure}{R}{0.5\textwidth}
    \centering
    \includegraphics[width=0.48\textwidth]{09_Multizone/Images/advection/adv_example.png}
...
\begin{equation}
    \begin{aligned}
    \qty(\pdv{n}{t})_\text{advection}&=-\nabla\cdot\qty(n\boldsymbol{v}_A)\text{ ,} 
...
\begin{equation}
    \begin{aligned}
    \qty(\pdv{n}{t})_\text{advection}&=-\boldsymbol{v}_A\nabla n\text{ .} 
...
\begin{equation}
    \begin{aligned}
    \qty(\pdv{n}{t})_\text{advection}\bigg|
        \begin{array}l
             \gamma \\
             t \\
             x,y,z
        \end{array}&=-v\frac{\Delta t}{\Delta x}\sum_\qty{i=x,y,z}
        \begin{cases}
            \qty(n_{i+\Delta i}^t-n_{i}^t)\text{,}& v_i<0 \\
            \qty(n_{i}^t-n_{i-\Delta i}^t)\text{,}& v_i>0
        \end{cases}\text{ ,} 
    \end{aligned} \label{eq:chapter_9_multizone_adv_numerical_sol}
\end{equation}
\noindent where the forward difference method (i.e. using cells $i$ and $i+\Delta i$ to approximate the derivative) is used when $v_i<0$ and the backward difference method (i.e. using cells $i$ and $i-\Delta i$) is used when $v_i>0$. 
\par~\par 
...
\begin{equation}
    \begin{aligned}
    \Delta t \leq \frac{\Delta x}{\abs{v}}\text{ .} 
...
\begin{equation}
    \begin{aligned}
        \pdv{n}{t}+v\pdv{n}{x}&= \frac{v\Delta x}{2}\pdv[2]{n}{x}\mp \frac{\Delta t}{2}\pdv[2]{n}{t} \text{ .} 
...
\begin{equation}
    \begin{aligned}
        \pdv[2]{n}{t}&= -v\pdv{}{t}\qty[\pdv{n}{x}] = -v\pdv{}{x}\qty[\pdv{n}{t}] =v^2\pdv[2]{n}{x}\text{ ,} 
    \end{aligned}
\end{equation}
\noindent and placing it into \autoref{eq:09_multizone_dispersion_ex3}:
\begin{equation}
    \begin{aligned}
        \pdv{n}{t}+v\pdv{n}{x}&= \frac{v\Delta x}{2}\pdv[2]{n}{x}\mp\frac{v^2\Delta t}{2}\pdv[2]{n}{x} \\
        &=\frac{v\Delta x}{2}\qty(1\mp v\frac{\Delta t}{\Delta x})\pdv[2]{n}{x} \\
        &=D' \pdv[2]{n}{x}\text{ ,} 
...
\begin{equation}
    \begin{aligned}
        D'&=\frac{v\Delta x}{2}\qty(1\mp v\frac{\Delta t}{\Delta x})\text{ ,} 
    \end{aligned}
\end{equation}
...
\begin{equation}
    \begin{aligned}
        n_\text{diff+adv}\qty(E,t,\boldsymbol{r},\boldsymbol{v})&=n_\text{diff}\qty(E,t,\boldsymbol{r}-\boldsymbol{v}t)\text{ ,} 
    \end{aligned} \label{eq:09_multizone_adv_2D_analytical}
\end{equation}
\noindent where $\boldsymbol{r}=\qty[x,y,z=50]$, The numerical solution is able to match adequatly the analytical solution as shown in the top panels of \autoref{fig:09_multizone_adv_2D}.
