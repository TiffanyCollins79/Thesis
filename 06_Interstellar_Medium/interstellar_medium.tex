...
\newpar 
\noindent Let a telescope with detecting area $\dd{A}$ recieves $dE$ of energy (in frequency range $\nu + \dd{\nu}$) from a source within solid angle $\dd{\Omega}$ at orientation $\theta$ in time $\dd{t}$ (see \autoref{fig:telescope_basic_diagram2}).
...
\end{equation}
\noindent where $I_{i}$ is the moment of inertia in the ith axis ($i=x,y,z$). For linear rotors (e.g. H$_2$, CO)  $I_x\ll I_y=I_z$ where the moment of inertial in one axes (e.g. $I_x$) can be treated as zero \citep{alma9928060792901811}. \autoref{eq:chapter_6_molecular_rotation_energy} becomes:
...
\begin{figure}[b!]
    ...
    \caption{(\textit{Left}) Four spherical gas clouds located $4\,\kpc$ from the Earth at distances $d$ along the line of sight (z-axis runs in/out of page) from a source of electrons (red cross). (\textit{Right}) Total gamma-ray flux (inverse Compton and Bremsstrahlung) $40\,\kiloyear$ after electron injection commences.}
