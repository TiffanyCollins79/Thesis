...
\end{equation}
\noindent where $\mathcal{D}\equiv D\Delta t/\Delta i^2$. %i.e. \autoref{eq:A2_diffusion_finite_diff} is the solution of \autoref{eq:A2_finite_difference_diffusion} at time $t+\Delta t$ using finite difference techniques.
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
\noindent where $\theta\equiv k\Delta i$. For the solution to be stable, $\lvert \frac{A^{t+1}}{A^t}\rvert\leq 1$. Hence:
...
\end{equation}
\noindent where $v'_i\equiv v_\text{A,i}{\Delta t}/{\Delta i}$. Applying the Von Neumann stability analysis to \autoref{eq:A2_advection_central_diff}:
